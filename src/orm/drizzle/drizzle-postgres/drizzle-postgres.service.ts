...
export class DrizzlePostgresService extends AbstractDrizzlerService {
	...
	> {
		const result = await this.driver.transaction(async (tx) => {
			...
			const employee_leaves = await tx
				.select()
			...
			const employee_salary = await tx
				.select()
			...
			return tx
				...
				.where(
					and(
						...
						eq(clientsPayments.client_payment_client_id, client_payment_id),
					),
				);
		});
